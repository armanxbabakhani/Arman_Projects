...
// LICENSE:       LGPL
// 
// AUTHOR:        Henry Pinkard and Arman Babakhani, hbp@berkeley.edu, armanxbabakhani@gmail.com 7/6/2017         
//
//
...
const char* g_DeviceNameLEDArray = "LED-Array";
	...
	const char * g_Keyword_Red = "Intensity_Red";  //Global intensity with a maximum of 65535
	const char * g_Keyword_Green = "Intensity_Green";  //Global intensity with a maximum of 65535
	const char * g_Keyword_Blue = "Intensity_Blue";  //Global intensity with a maximum of 65535
	const char * g_Keyword_SingleLED = "SingleLED"; // Lighting single LED
	const char * g_Keyword_MultipleLEDs = "MultipleLEDs"; // Lighting multiple LEDs
	...
	const char * g_Keyword_type = "PatternType"; //Pattern type: top, bottom, left, right
	const char * g_Keyword_minna = "MinimumNA"; 
	const char * g_Keyword_maxna = "MaximumNA";
	const char * g_Keyword_LEDlist = "ManualLEDIndices";
	const char * g_Keyword_ArrayDist = "ArrayDistance";
...
CLEDArray::CLEDArray() : initialized_(false), name_(g_DeviceNameLEDArray), pixels_(0), width_(1), height_(581),
	shutterOpen_(false), red_(600), green_(400), blue_(200), numa_(0.7), minna_(0.2), maxna_(0.6), type_("Top"), dist_(0)
{
...
int CLEDArray::Initialize()
{
   ...
   AddAllowedValue(g_Keyword_Pattern, "ColoredDPC");
   AddAllowedValue(g_Keyword_Pattern, "ManualLEDIndices");
   AddAllowedValue(g_Keyword_Pattern, "Annulus");
   AddAllowedValue(g_Keyword_Pattern, "HalfAnnulus");
   ...
   CreateProperty(g_Keyword_minna, "", MM::Float, false, pActmin);

   CPropertyAction* pActmax = new CPropertyAction(this, &CLEDArray::OnMaxNA);
   CreateProperty(g_Keyword_maxna, "", MM::Float, false, pActmax);
   ...
   //Set Array Distance:
   CPropertyAction* pActdist = new CPropertyAction(this, &CLEDArray::OnDist);
   CreateProperty(g_Keyword_ArrayDist, "", MM::Float, false, pActdist);
...
int CLEDArray::Dist(double dist){
	PurgeComPort(port_.c_str());
	int i, size, d100 = 100*dist, DIST[8]; // na100 is 100*na which is the required format for the input command.
	unsigned char allData[100];
	    
	std::string distance = std::to_string(static_cast <long long> (d100));
	std::copy(distance.begin(),distance.end(),DIST);
	allData[0] = 's';
	allData[1] = 'a';
	allData[2] = 'd';
	allData[3] = ',';
...
int CLEDArray::OnPattern(MM::PropertyBase* pProp, MM::ActionType pAct)
{
   ...
   else if (pAct == MM::AfterSet)
   {
	  ...
	  else if(pattern_ == "ColoredDPC"){
		  NumA(numa_);
		  ColorUpdate(red_,green_,blue_);
		  CDPC(red_,green_,blue_);
	  }
	  else if(pattern_ == "ManualLEDIndices"){
		  if (indices_.size() > 0){
			ColorUpdate(red_,green_,blue_);
			MLED(indices_);
		  }
...
int CLEDArray::OnDist(MM::PropertyBase* pProp, MM::ActionType pAct)
{
	if (pAct == MM::BeforeGet)
	{
		pProp->Set(dist_);
